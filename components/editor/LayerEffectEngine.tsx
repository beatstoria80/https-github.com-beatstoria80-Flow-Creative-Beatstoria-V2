...
export const LayerEffectEngine: React.FC<LayerEffectEngineProps> = ({ config, setConfig, selectedId }) => {
  ...
  const updateEffect = (key: keyof LayerEffects, value: any, save = false) => {
    setConfig(prev => {
      // Find the specific layer in the prev state to avoid stale effects
      let targetEffects: LayerEffects = DEFAULT_EFFECTS;
      if (selectedId === 'headline') targetEffects = prev.typography.headline_effects || DEFAULT_EFFECTS;
      else if (selectedId === 'subtitle') targetEffects = prev.typography.subtitle_effects || DEFAULT_EFFECTS;
      else {
        const l = [...prev.image_layers, ...prev.additional_texts, ...(prev.shapes || [])].find(l => l.id === selectedId);
        if (l && (l as any).effects) targetEffects = (l as any).effects;
      }

      const updated = { ...targetEffects, [key]: value };
