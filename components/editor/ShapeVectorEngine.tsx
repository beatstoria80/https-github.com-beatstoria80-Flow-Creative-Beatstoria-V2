...
export const ShapeVectorEngine: React.FC<ShapeVectorEngineProps> = ({ config, setConfig, onSelectLayer, selectedId }) => {
    ...
    const handleAddShape = (type: ShapeLayer['shape_type']) => {
        ...
        setConfig(prev => {
            const containerW = Number(prev.canvas?.width) || 1080;
            const containerH = Number(prev.canvas?.height) || 1080;

            const newShape: ShapeLayer = {
                id: newId,
                shape_type: type,
                position_x: Math.round(containerW / 2 - 100),
                position_y: Math.round(containerH / 2 - 100),
                width: 200,
                height: 200,
                rotation: 0,
