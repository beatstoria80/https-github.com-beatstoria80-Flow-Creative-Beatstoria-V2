...
export const MaskEngine: React.FC<MaskEngineProps> = ({ config, setConfig, selectedId }) => {
    ...
    const handleToggleMask = () => {
        setConfig(prev => {
            const l = prev.image_layers.find(x => x.id === selectedId);
            if (!l) return prev;
            const newState = !l.mask_enabled;

            if (newState && !l.mask_src) {
                ...
                return {
                    ...prev,
                    image_layers: prev.image_layers.map(x => x.id === selectedId ? {
                        ...x,
                        mask_enabled: true,
                        mask_src: newMaskSrc,
            ...
            return {
                ...prev,
                image_layers: prev.image_layers.map(x => x.id === selectedId ? { ...x, mask_enabled: newState } : x)
            };
        }, true);
    };
    ...
    const handleParamChange = (key: string, val: any, save = false) => {
        setConfig(prev => {
            ...
            const type = key === 'mask_type' ? val : (l.mask_type || 'circle');
            const feather = key === 'mask_feather' ? val : (l.mask_feather || 0);
            const inverted = key === 'mask_inverted' ? val : (!!l.mask_inverted);
