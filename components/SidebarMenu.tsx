...
import {
    ...
    Save, LogIn, ChevronLeft, AlertTriangle, HardDriveDownload, Gauge, Eraser
} from 'lucide-react';
...
const ProjectCard = ({ name, date, active, onClick, onDelete, onExport, id }: any) => {
    ...
    return (
        <div
            ...
            className={`group relative flex items-center gap-3 p-4 rounded-[1.5rem] border transition-all duration-500 cursor-pointer ${active
        ...
        >
            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 transition-all duration-500 ${deleteStage === 'confirm' ? 'bg-red-500 text-white animate-pulse' :
                active ? 'bg-indigo-600 text-white rotate-6' : 'bg-slate-50 text-slate-400 group-hover:bg-indigo-50 group-hover:text-indigo-600'
                }`}>
                {deleteStage === 'confirm' ? <AlertTriangle size={20} strokeWidth={2.5} /> : <span className="text-sm font-black uppercase tracking-widest">{name ? name.substring(0, 2) : 'UN'}</span>}
            </div>
            ...
            <div className="flex items-center gap-2 relative z-30" onClick={(e) => e.stopPropagation()}>
                <div className={`flex items-center gap-1.5 ${active || deleteStage === 'confirm' ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'} transition-opacity`}>
                    ...
                    <button
                        ...
                        className={`flex items-center gap-2 p-2 rounded-xl transition-all duration-300 ${deleteStage === 'confirm'
...
export const SidebarMenu: React.FC<SidebarMenuProps> = ({
    ...
    return (
        <>
            ...
            <div
                className={`fixed top-4 left-4 bottom-4 w-[420px] bg-white z-[4001] shadow-[0_30px_100px_rgba(0,0,0,0.3)] rounded-[2.5rem] transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] transform ${isOpen ? 'translate-x-0 opacity-100' : '-translate-x-full opacity-0 pointer-events-none'} overflow-hidden flex flex-col border border-slate-200/50`}
            >
                {/* TOP BAR */}
                                            ...
                                            <div className="grid grid-cols-4 gap-1.5">
                                                {(['DRAFT', 'IN_PROGRESS', 'REVIEW', 'APPROVED'] as FrameStatus[]).map(status => (
                                                    <button
                                                        ...
                                                        className={`py-2 rounded-xl text-[7px] font-black uppercase tracking-wider transition-all border ${config.canvas.status === status
